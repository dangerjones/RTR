...
if(!isset($_POST['type'])) {
	...
	switch($type) {
		case 'Approve':
			$util->changeEStatus($e_id, ESTATUS_OK);
			$mail->notifyStatusChange($e_id, ESTATUS_OK);
			break;
		case 'Deny':
			$util->changeEStatus($e_id, ESTATUS_DENY);
			$mail->notifyStatusChange($e_id, ESTATUS_DENY);
			break;
		case 'Delete':
			$util->deleteEvent($e_id);
			break;
		default:
