...
class Event {
	...
	function getPermalinkURL() {
		return BASEURL . $this->getPermalink();
	}
	function getfDesc() {
    ...
    function formattedAsList() {
		...
		if($this->reg_here || $this->hasRaceResults()) {
			...
			if($this->reg_here && !$this->hasPassed())
				$output .= '<a href="/register/'. $this->getPermalink() .'">register</a>';
			//<img src="/img/document-icon.png" alt="Register online" title="Register online" />

			if($this->hasRaceResults())
				$output .= 'results';//$this->raceResultsImage();
    ...
    function formattedContent() {
		...
		$output .= ' <a href="/events/'. $this->getPermalink() .'" title="Event\'s page">';
		$output .= BASEURL .'events/'. $this->getPermalink() .'</a>';
		$output .= '</div>';
